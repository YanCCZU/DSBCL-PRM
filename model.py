...
class TrajectoryModel(nn.Module):
    ...
    def __init__(self, in_size, obs_len, pred_len, embed_size, enc_num_layers, int_num_layers_list, heads, forward_expansion):
        ...
        self.embedding1 = nn.Linear(in_size * 19 , embed_size)
        #self.embedding2 = nn.Linear(in_size * 7, embed_size)
        #self.vel_enc = nn.GRU(2, embed_size, batch_first=True)
        #self.embedding2 = nn.Linear(in_size * pred_len, 32)
        self.mlp = nn.Sequential(
            nn.Linear(embed_size + 24 ,embed_size),
            nn.ReLU(),
            nn.Linear(embed_size, 24)
        )
        ...
        self.nei_embedding1 = nn.Linear(in_size*(obs_len + pred_len), embed_size)
    ...
    def forward(self, ped_obs, neis_obs, motion_modes, mask, closest_mode_indices, test=False, num_k=20):
        ...
        ped_seq = ped_seq[:,:,1:,:] - ped_seq[:,:,:-1,:]  #vel
        ped_seq = ped_seq.reshape(ped_seq.shape[0], ped_seq.shape[1], -1)  # [B K seq_len*2]
        ped_embedding = self.embedding1(ped_seq) # [B K embed_size]
        ...
        if not test:
            ...
            pred_traj_vel = self.reg_head(int_feat.squeeze())  # [B pred_len*2]

            diff_feat = torch.cat((int_feat.squeeze(), pred_traj_vel), dim=-1)
            diff = self.mlp(diff_feat)
            pred_traj_ref_vel = pred_traj_vel + diff

            return pred_traj_vel,pred_traj_ref_vel, scores, int_feat
        ...
        if test:
            ...
            pred_trajs_vel = self.reg_head(int_feats)  # [B num_k pred_size*2]

            diff_feats = torch.cat((int_feats, pred_trajs_vel), dim=-1)
            diffs = self.mlp(diff_feats)
            pred_trajs_ref_vel = pred_trajs_vel + diffs


            return pred_trajs_ref_vel, scores,top_k_feat#pred_trajs_vel
